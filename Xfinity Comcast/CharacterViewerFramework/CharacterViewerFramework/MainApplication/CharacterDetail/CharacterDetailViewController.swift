...
class CharacterDetailViewController: UIViewController {
    ...
    func setUpData() {
        ...
        if let urlString = relatedTopic.icon?.url, let url = URL(string: urlString) {
            characterImageView.sd_setImage(with: url, placeholderImage: UIImage(named: Constant.DefaultStrings.defaultImageName, in: CharacterViewerBundleHelper.bundle, compatibleWith: nil))
        }
